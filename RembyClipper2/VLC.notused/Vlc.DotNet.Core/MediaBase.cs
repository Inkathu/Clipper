...
namespace Vlc.DotNet.Core
{
    public abstract class MediaBase : IDisposable
    {
        ...
        private static IntPtr _myVlcMediaPlayerEventManager;

        private IntPtr myVlcMediaPlayerEventManager { get { return _myVlcMediaPlayerEventManager; } set { _myVlcMediaPlayerEventManager = value; } }
        ...
        static LibVlcMethods.EventCallbackDelegate callback;
        protected internal virtual IntPtr Initialize(IntPtr vlcClient)
        {
            if (VlcMedia == IntPtr.Zero)
                return IntPtr.Zero;
            MediaBase.callback = OnVlcEvent;
            myEventCallback = Marshal.GetFunctionPointerForDelegate(MediaBase.callback);
            GC.KeepAlive(MediaBase.callback);
